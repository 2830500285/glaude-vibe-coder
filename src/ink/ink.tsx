...
export default class Ink {
  ...
  // "current" (yellow). null clears. Positions are known upfront 閳?  // navigation is index arithmetic, no scan-feedback loop.
  ...
  // so App.tsx's handleMouseEvent is stateless 閳?dispatchHover diffs
  ...
  // blit 閳?selection overlay mutated it, resetFramesForAltScreen()
  // replaced it with blanks, or forceRedraw() reset it to 0鑴?. Forces
  ...
  // screen readers / screen magnifiers track it 閳?so parking at the text
  ...
  // this 閳?every frame begins with CSI H. null = no move emitted last frame.
  ...
  constructor(private readonly options: Options) {
    ...
    // state set in layout effects 閳?notably the cursorDeclaration from
    // useDeclaredCursor 閳?would lag one commit behind if we rendered
  ...
  // but this.terminalColumns/Yoga are OLD 閳?any scheduleRender during that
  ...
  // blank閳姫aint flicker). useVirtualScroll's height scaling already bounds
  ...
  private handleResize = () => {
    ...
    // scratch (prevFrameContaminated 閳?every cell written, wrapped in
    // BSU/ESU 閳?old content stays visible until the new frame swaps
    ...
    // buffer clear even when already in alt 閳?that's the blank flicker.
  ...
  enterAlternateScreen(): void {
    ...
    // Disable extended key reporting first 閳?editors that don't speak
   ...
   * and subsequent renders land in main 閳?native terminal scroll
   * returns, fullscreen scroll is dead.
   */
  exitAlternateScreen(): void {
    this.options.stdout.write((this.altScreenActive ? ENTER_ALT_SCREEN : '') +
    // re-enter alt 閳?vim's rmcup dropped us to main
    '\x1b[2J' +
    ...
    this.resume();
    // Re-enable focus reporting and extended key reporting 閳?terminal
  ...
  onRender() {
    ...
    // Entering a render cancels any pending drain tick 閳?this render will
    ...
    // TEXT (native terminal behavior 閳?the selection walks up the screen
    ...
    // before they're overwritten 閳?the text stays copyable until the
    // selection scrolls entirely off. During drag, focus tracks the mouse
    // (screen-local) so only anchor shifts 閳?selection grows toward the
    ...
    // in the footer/prompt/StickyPromptHeader are on static text 閳?the
    ...
    this.selection.anchor.row >= follow.viewportTop && this.selection.anchor.row <= follow.viewportBottom) {
      ...
      // frame and scrolledOffAbove grows without bound 閳?getSelectedText
      ...
      // 閳?shiftSelectionForFollow would clamp it to viewportBottom,
      ...
      // contents 閳?no accumulation. Dragging branch doesn't need this:
      ...
      !this.selection.focus || this.selection.focus.row >= viewportTop && this.selection.focus.row <= viewportBottom) {
        ...
        // fire the listeners directly 閳?they schedule a React update for
    ...
    // (spinner appears 閳?bottom grows 閳?scrollbox shrinks), the
    ...
    // actually SHIFTS 閳?didLayoutShift() tracks exactly this (any node's
    ...
    if (this.altScreenActive) {
      ...
      // rowOffset. No scanning 閳?positions came from a prior scan when
    ...
    // can't do this 閳?cursor.y tracks scrollback rows CSI H can't reach.
    // The CSI H write is deferred until after the diff is computed so we
    // can skip it for empty diffs (no writes 閳?physical cursor unused).
    ...
    // DECSTBM needs BSU/ESU atomicity 閳?without it the outer terminal
    ...
    if (this.altScreenActive && hasDiff) {
      ...
      // row (where the prompt input is) 閳?without this, the cursor ends
      ...
      // 閳?but the physical terminal still has stale content there (shorter
    ...
    // translation) 閳?if the declared node didn't render (stale declaration
    ...
    if (hasDiff || targetMoved || target === null && parked !== null) {
      ...
      if (target !== null) {
        ...
        // to frame.cursor before forgetting the park position 閳?otherwise
    ...
    // are only ever true in alt-screen; in main-screen this is false閳姁alse.
    this.prevFrameContaminated = selActive || hlActive;

    // A ScrollBox has pendingScrollDelta left to drain 閳?schedule the next
    ...
    // of this window) 閳?leadingEdge fires IMMEDIATELY 閳?double render ~0.1ms
    // apart 閳?jank. Use a plain timeout. If a wheel event arrives first,
    // its scheduleRender path fires a render which clears this timer at
    // the top of onRender 閳?no double.
    //
   ...
   *
   * The traditional readline ctrl+l 閳?clears the visible screen and
  ...
  forceRedraw(): void {
    ...
    if (this.altScreenActive) {
      ...
      this.repaint();
      // repaint() resets frontFrame to 0鑴?. Without this flag the next
   ...
   *
   * Lighter than forceRedraw() 閳?no screen clear, no extra write. Call
   ...
   * stall). Catches tmux detach閳妺ttach, ssh reconnect, and laptop
   * sleep/wake 閳?none of which send SIGCONT. The terminal may reset DEC
   ...
   * Kitty keyboard protocol is NOT 閳?CSI >1u is a stack push, so we pop
   * first to keep depth balanced (pop on empty stack is a no-op per spec,
   * so after a terminal reset this still restores depth 0閳?). Without the
   * pop, each >5s idle gap adds a stack entry, and the single pop on exit
   * or suspend can't drain them 閳?the shell is left in CSI u mode where
   * Ctrl+C/Ctrl+D leak as escape sequences. The alt-screen
   * re-entry (ERASE_SCREEN + frame reset) is NOT idempotent 閳?it blanks the
   * screen 閳?so it's opt-in via includeAltScreen. The stdin-gap caller fires
  ...
  reassertTerminalModes = (includeAltScreen = false): void => {
    ...
    // Don't touch the terminal during an editor handoff 閳?re-enabling kitty
    // keyboard here would undo enterAlternateScreen's disable and nano would
    // start seeing CSI-u sequences again.
    if (this.isPaused) return;
    // Extended keys 閳?re-assert if enabled (App.tsx enables these on
    ...
    // Mouse tracking 閳?idempotent, safe to re-assert on every stdin gap.
    if (this.altScreenMouseTracking) {
      this.options.stdout.write(ENABLE_MOUSE_TRACKING);
    }
    // Alt-screen re-entry 閳?destructive (ERASE_SCREEN). Only for callers that
   ...
   * cleanup block + updateContainerSync 閳?AlternateScreen unmount cleanup.
   ...
   * as restoring the saved cursor position 閳?clobbering the resume hint.
   */
  detachForShutdown(): void {
    ...
    // unmount (App.componentWillUnmount 閳?handleSetRawMode(false)) but we're
    // short-circuiting that path. Must use this.options.stdin 閳?NOT
    // process.stdin 閳?because getStdinOverride() may have opened /dev/tty
    // when stdin is piped.
   ...
   * SIGCONT, resize, and stdin-gap/event-loop-stall (sleep/wake) 閳?any of
  ...
  /**
   * Seed prev/back frames with full-size BLANK screens (rows鑴砪ols of empty
   * cells, not 0鑴?). In alt-screen mode, next.screen.height is always
   ...
   *
   * With a rows鑴砪ols blank prev, heightDelta === 0 閳?standard diffEach
   * 閳?moveCursorTo (CSI cursorMove, no LF, no scroll).
   *
  ...
  private resetFramesForAltScreen(): void {
    ...
    this.displayCursor = null;
    // Fresh frontFrame is blank rows鑴砪ols 閳?blitting from it would copy
  ...
  copySelectionNoClear(): string {
    ...
    if (text) {
      ...
      // drops it silently unless allow-passthrough is on 閳?no regression).
  ...
  /**
   * Set the search highlight query. Non-empty 閳?all visible occurrences
   * are inverted (SGR 7) on the next frame; first one also underlined.
   * Empty 閳?clears (prevFrameContaminated handles the frame after). Same
   * damage-tracking machinery as selection 閳?setCellStyleId doesn't track
   ...
   *
   *  The element comes from the MAIN tree 閳?built with all real
   ...
   *
   *  ~1-2ms (paint only, no reconcile 閳?the DOM is already built). */
  scanElementSubtree(el: dom.DOMElement): MatchPosition[] {
    ...
    // Passing -elLeft/-elTop nets to 0 閳?paints at (0,0) in our buffer.
    ...
    // renderNodeToOutput wrote our offset positions to nodeCache 閳?    // corrupts the main render (it'd blit from wrong coords). Mark the
    ...
    logForDebugging(
      `scanElementSubtree: q='${this.searchHighlightQuery}' ` +
        ...
        `[${positions.slice(0, 10).map(p => `${p.row}:${p.col}`).join(',')}` +
        `${positions.length > 10 ? ',...' : ''}]`,
    );
    return positions;
  }
   ...
   *  highlight (inverse on all matches) still runs 閳?this overlays yellow
   ...
   * (rgb(), ansi:name, #hex, ansi256()) 閳?colorize() routes through
  ...
  setSelectionBgColor(color: string): void {
    ...
    // colorize returns the input unchanged if the color string is bad 閳?    // no NUL-split then, so fall through to null (inverse fallback).
   ...
   * this moves BOTH endpoints 閳?the user isn't holding the mouse at one
  ...
  shiftSelectionForScroll(dRow: number, minRow: number, maxRow: number): void {
    ...
    // useHasSelection updates. Safe to call notifySelectionChange here 閳?    // this runs from keyboard handlers, not inside onRender().
   ...
   * Left/right wrap across row boundaries 閳?native macOS text-edit
   ...
   * altScreenActive 閳?clicks only make sense with a fixed viewport where
  ...
  dispatchKeyboardEvent(parsedKey: ParsedKey): void {
    ...
    // Tab cycling is the default action 閳?only fires if no handler
   ...
   * with no side effects 閳?call it synchronously at click time so the
   ...
   * the mutable field at call time 閳?not the undefined-at-render value.
   */
  ...
  // cascades through useContext 閳?<AlternateScreen>'s useLayoutEffect dep
  // array 閳?spurious exit+re-enter of the alt screen on every SIGWINCH.
  ...
  unmount(error?: Error | number | null): void {
    ...
    if (this.options.stdout.isTTY) {
      ...
      // Disable mouse tracking 閳?unconditional because altScreenActive can be
   ...
   * migration 閳?resetScreen zeros it before any reads.
   *
   ...
   * hooks console.* methods 閳?direct stderr writes bypass it, land at the
   ...
   * absolute coords 閳?interleaved garbage.
   *
   ...
   * process.stdout 閳?Ink itself writes there.
   */
  private patchStderr(): () => void {
    ...
    const intercept = (chunk: Uint8Array | string, encodingOrCb?: BufferEncoding | ((err?: Error) => void), cb?: (err?: Error) => void): boolean => {
      ...
      // Reentrancy guard: logForDebugging 閳?writeToStderr 閳?here. Pass
 ...
 *
 * 1. setRawMode is termios, not fcntl 閳?the stdin fd stays blocking, so
...
export function drainStdin(stdin: NodeJS.ReadStream = process.stdin): void {
  ...
  // returns null when empty 閳?never blocks.
  try {
  ...
  // reads (64KB) 閳?a real mouse burst is a few hundred bytes; the cap
  // guards against a terminal that ignores O_NONBLOCK.
  let fd = -1;
  try {
    // setRawMode inside try: on revoked TTY (SIGHUP/SSH disconnect) the
    // ioctl throws EBADF 閳?same recovery path as openSync/readSync below.
    ...
    // EAGAIN (buffer empty 閳?expected), ENXIO/ENOENT (no controlling tty),
    // EBADF/EIO (TTY revoked 閳?SIGHUP, SSH disconnect)
  } finally {
    if (fd >= 0) {
      try {
        closeSync(fd);
