...
export async function fileHistoryTrackEdit(
  ...
  // the deterministic {hash}@v1 backup on every repeat call 鈥?a second
  ...
  updateFileHistoryState((state: FileHistoryState) => {
    try {
      ...
      // backup's Date/string fields 鈥?O(n) cost to add one entry.
...
export async function fileHistoryMakeSnapshot(
  ...
  // 鈥?if fileHistoryTrackEdit added a file during phase 2's async window, it
...
function compareStatsAndContent<T extends boolean | Promise<boolean>>(
  ...
  // own read errors 鈥?a try/catch here is dead for async callbacks anyway.
...
async function createBackup(
  ...
  // copy. Separates "source missing" from "backup dir missing" cleanly 鈥?  // sharing a catch for both meant a file deleted between copyFile-success
